...
- Step 0 blockers are still open: Intentwise OAuth credentials, Emplicit Postgres dev DB, Teamwork API token, and NotebookLM Enterprise API access are all pending. Do not recommend steps that require these until the user confirms credentials have arrived — use `/intentwise-ready` or `/notebooklm-ready` skills for that.
