...
```bash
echo -n "your-value" | gcloud secrets create ANTHROPIC_API_KEY --data-file=- --project=$PROJECT_ID
echo -n "your-value" | gcloud secrets create EMPLICIT_PG_HOST --data-file=- --project=$PROJECT_ID
echo -n "your-value" | gcloud secrets create EMPLICIT_PG_PORT --data-file=- --project=$PROJECT_ID
echo -n "your-value" | gcloud secrets create EMPLICIT_PG_DB --data-file=- --project=$PROJECT_ID
echo -n "your-value" | gcloud secrets create EMPLICIT_PG_USER --data-file=- --project=$PROJECT_ID
echo -n "your-value" | gcloud secrets create EMPLICIT_PG_PASSWORD --data-file=- --project=$PROJECT_ID
echo -n "your-value" | gcloud secrets create SLACK_BOT_TOKEN --data-file=- --project=$PROJECT_ID
...
Set the time to run **after Intentwise completes its daily sync** (confirm exact sync completion
time with the data team before setting this).
...
# Create the schedule — 9:00 AM ET daily (adjust once Intentwise sync time confirmed)
# april13 waiting on confirmation - what time does Intentwise complete its daily sync?
#   Set this to ~30 min after sync completes to ensure all data is available.
...
> `0 14 * * *` = 9:00 AM ET (UTC-5 in winter). Adjust for daylight saving or confirm with data team.
